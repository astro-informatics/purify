...
def test_deconvolution_kernel():
    ...
    oversampling = 2, 2
    interpolation = 4, 4
    kernels = create_kernels(visibility, dimensions, oversampling, interpolation)
    ...
    xaxis = axis(dimensions[0], oversampling[0], interpolation[0])
    yaxis = axis(dimensions[1], oversampling[1], interpolation[1])
...
def visibility_image_operator():
    ...
    from os.path import join, dirname
    from purify import MeasurementOperator, read_visibility, Image, __file__ as path

    visibility_path = join(dirname(path), "data", "images", "Coverages", "cont_sim4.vis")
    ...
    dimensions = nearest_power_of_two(image.shape[0]), nearest_power_of_two(image.shape[1])
    oversampling = 2, 2
    interpolation = 4, 4
    operator = MeasurementOperator(visibility, dimensions, oversampling, interpolation)
...
def test_harden_forward_measurement_operator():
    ...
    expected = [  2.03724422e-04 +4.15991536e-04j,  1.08735157e-03 -6.79113430e-03j,
                 -1.60430819e-02 +1.60710447e-01j, -7.38552963e-04 -1.19728759e-03j,
                  2.97760267e-05 -1.82804158e-04j,  7.26545343e-03 -5.90595568e-03j,
                 -6.70682680e-04 -5.40322258e-04j,  1.02806690e-04 +2.30846267e-05j,
                  1.33592565e-03 +3.61525331e-03j,  2.80500284e-02 -2.41679083e-02j,
                 -5.99541058e-04 +2.08489869e-03j, -5.95013573e-04 +1.53026842e-03j,
                 -2.22493322e-03 -1.41642788e-03j,  1.81243121e-03 -4.02146676e-03j ]
