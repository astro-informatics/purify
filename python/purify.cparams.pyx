...
cdef double guess_gamma_penalty(
        sdmm, visibility, MeasurementOperator measurements ):
    ...
    cdef double npixels = product(measurements.sizes.image)
...
cdef void _convert_l1param( sopt_l1_sdmmparam* c_params, sdmm,
    ...
    c_params.gamma = sdmm.gamma if sdmm.gamma is not None \
        else guess_gamma_penalty(sdmm, visibility, measurements)
    c_params.epsilon = sdmm.radius if sdmm.radius is not None \
        else guess_radius(sdmm, visibility, measurements)
...
cdef void _convert_rwparams( sopt_l1_rwparam *c_params, sdmm,
    ...
    else: c_params.sigma = guess_sigma(sdmm, visibility, measurements)
