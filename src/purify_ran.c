 ...
 * \note This routine calls \ref purify_ran_ran2 and so changes the
 * state of the random generator.
 *
 * \warning This C implementation has been copied from a Fortran
 * version and has not yet been tested!
 *
 * \warning Must inialise \ref purify_ran_ran2 first with a negative
 * seed first and then subsequently call with the identical magnitude
 * positive seed, i.e. if seed>0, initialise \ref purify_ran_ran2 with
...
double purify_ran_gasdev2(int idum) {
  ...
  if (iset ==0) {
    do {
      v1=2.*purify_ran_ran2(idum)-1.;
      v2=2.*purify_ran_ran2(idum)-1.;
      rsq=v1*v1+v2*v2;
...
int purify_ran_knuthshuffle(int *perm, int nperm, int N, int seed) {
  ...
  // Check inputs.
  if (nperm > N) {
    PURIFY_ERROR_GENERIC("Number of required permutations greater than number of integers");
    return 1;
  }
      
  // Allocate scratch space.
  scratch = (int*)malloc(N * sizeof(int));
  PURIFY_ERROR_MEM_ALLOC_CHECK(scratch);
  ...
  for (i = 0; i < nmax; i++) {
    ...
    j = (int)(purify_ran_ran2(seed) * (N - i));
    j = j + i;
