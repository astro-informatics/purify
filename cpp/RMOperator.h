  ...
  class RMOperator {
   public:
      ...
      RMOperator(const utilities::rm_params& rm_vis_input, const t_int & J, 
        ...
        const t_real & cell_size = 1, const std::string& weighting_type = "none", const t_real& R = 0, bool fft_grid_correction = false);
    ...
    protected:
      ...
      Sparse<t_complex> init_interpolation_matrix1d(const Vector<t_real>& u, const t_int Ju, const std::function<t_real(t_real)> kernelu);    
      ...
      Array<t_complex> init_weights(const Vector<t_real>& u, const Vector<t_complex>& weights, const t_real & oversample_factor, const std::string& weighting_type, const t_real& R);
