...
TEST_CASE("Serial vs Distributed Operator") {
  ...
  auto op = std::make_shared<sopt::LinearTransform<Vector<t_complex>>>(
      sopt::linear_transform<t_complex>(distribute_sara, imsizey, imsizex, world));
  auto factory_op = factory::wavelet_operator_factory<Vector<t_complex>>(
      factory::distributed_wavelet_operator::mpi_sara, wavelets, imsizey, imsizex);

  SECTION("forward") {
    const Vector<t_complex> input =
        Vector<t_complex>::Random(distribute_sara.size() * imsizex * imsizey);
    CAPTURE(input.size());
