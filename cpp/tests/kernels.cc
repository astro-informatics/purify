...
TEST_CASE("Measurement Operator [Kernels]", "[Kernels]") {
  std::vector<t_real> x_values = {-3,
                       ...
                       2.99000000000000,
                       3};
  std::vector<t_real> pswf_values = {0,
                          ...
                          2.86633764298305e-05,
                          0};
  std::vector<t_real> kb_values = {7.43469443502669e-06,
  ...
  std::vector<t_real> gauss_values = {0.000699879769063488,
...
std::vector<t_real> gauss_values
  ...
  for(t_int i = 0; i < number_of_samples; ++i) {
    ...
    t_real pswf_difference = std::abs(kernels::ft_pswf(x_values.at(i), 6) - pswf_values.at(i));
    CHECK(pswf_difference < 1e-13); // Test that pswf is the same as matlab calculations

    t_real kb_difference = std::abs(kernels::ft_kaiser_bessel(x_values.at(i), 6) - kb_values.at(i));
    CHECK(kb_difference < 1e-13); // Test that kaiser_bessel is the same as matlab calculations

    t_real gauss_difference = std::abs(kernels::ft_gaussian(x_values.at(i), 6) - gauss_values.at(i));
