...
TEST_CASE("Serial vs Distributed Operator Radial WProjection") {
  ...
  const auto op_serial = measurementoperator::init_degrid_operator_2d<Vector<t_complex>>(
      uv_serial, height, width, cellx, celly, over_sample, 10000, 1e-5, kernel, J, 4, abs_error,
      rel_error);

  const auto op = measurementoperator::init_degrid_operator_2d<Vector<t_complex>>(
      world, uv_mpi, height, width, cellx, celly, over_sample, 10000, 1e-5, kernel, J, 4, abs_error,
      rel_error);
