...
namespace purify {
  ...
  template <class T0>
    Sparse<typename T0::Scalar> compress_outer(Eigen::SparseMatrixBase<T0> const &matrix) {
      ...
      return  Eigen::MappedSparseMatrix<typename T0::Scalar>(matrix.rows(), indices.size(), matrix.nonZeros(), rows.data(),
          cols.data(), const_cast<typename T0::Scalar *>(matrix.derived().valuePtr()));
    }
} // namespace purify
#endif
