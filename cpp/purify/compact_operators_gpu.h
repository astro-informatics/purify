...
namespace purify {
  namespace gpu {
    namespace details {
      sopt::OperatorFunction<af::array>
        init_af_gridding_degridding_matrix_2d(const Vector<t_real> &u, const Vector<t_real> &v,
      ...
      sopt::OperatorFunction<Vector<t_complex>>
        init_af_gridding_degridding_matrix_2d(const sopt::mpi::Communicator &comm, const Vector<t_real> &u,
    ...
    namespace operators {
      sopt::OperatorFunction<Vector<t_complex>>
        base_grid_degrid_operator_2d(const Vector<t_real> &u, const Vector<t_real> &v, const Vector<t_real> &w,
          ...
          std::tie(directFZ, indirectFZ)
            = gpu::operators::af_base_padding_and_FFT_2d(ftkernelu, ftkernelv, imsizey, imsizex, oversample_ratio);
          ...
          const auto GTG = details::init_af_gridding_degridding_matrix_2d(u, v, w, weights, imsizey, imsizex, oversample_ratio, kernelv, kernelu, Ju, Jv, w_term, cellx,
...
#endif
      std::shared_ptr<sopt::OperatorFunction<Vector<t_complex>> const>
        init_grid_degrid_operator_2d(const Vector<t_real> &u, const Vector<t_real> &v, const Vector<t_real> &w,
            const Vector<t_complex> &weights, const t_uint &imsizey,
            const t_uint &imsizex, const t_real &oversample_ratio = 2,
          ...
          sopt::OperatorFunction<Vector<t_complex>> direct = purify::gpu::operators::base_grid_degrid_operator_2d(
              u, v, w, weights, imsizey, imsizex, oversample_ratio, kernel, Ju, Jv, w_term,
              energy_chirp_fraction, energy_kernel_fraction);
          const t_real op_norm
            = purify::details::power_method<Vector<t_complex>>({direct, N, [](Vector<t_complex> & out, const Vector<t_complex> & in){out = in;}, N}, power_iters, power_tol, Vector<t_complex>::Random(imsizex * imsizey));
          ...
          return std::make_shared<sopt::OperatorFunction<Vector<t_complex>> const>(direct);
        }
      std::shared_ptr<sopt::OperatorFunction<Vector<t_complex>> const>
        init_grid_degrid_operator_2d(const utilities::vis_params &uv_vis_input, const t_uint &imsizey,
          ...
          return gpu::operators::init_grid_degrid_operator_2d(
              ...
              power_tol, kernel, Ju, Jv, w_term, cell_x, cell_y, energy_chirp_fraction,
              energy_kernel_fraction);
        }

#ifdef PURIFY_MPI
      std::shared_ptr<sopt::OperatorFunction<Vector<t_complex>> const>
        init_grid_degrid_operator_2d_mpi(
            ...
            const t_uint Jv = 4, const bool w_term = false, const t_real cellx = 1, const t_real &celly = 1,
          ...
          std::array<t_int, 3> N = {0, 1, static_cast<t_int>(imsizey * imsizex)};
          std::array<t_int, 3> M = {0, 1, static_cast<t_int>(u.size())};
          sopt::OperatorFunction<Vector<t_complex>> direct = purify::gpu::operators::base_mpi_grid_degrid_operator_2d(
          ...
          const t_real op_norm
            = purify::details::power_method<Vector<t_complex>>({direct, N, [](Vector<t_complex> & out, const Vector<t_complex> & in){out = in;}, N}, power_iters, power_tol, Vector<t_complex>::Random(imsizex * imsizey));
          auto operator_norm = purify::operators::init_normalise<Vector<t_complex>>(op_norm * op_norm);
          return std::make_shared<sopt::OperatorFunction<Vector<t_complex>> const>(sopt::chained_operators<Vector<t_complex>>(direct, operator_norm));
        }
      std::shared_ptr<sopt::OperatorFunction<Vector<t_complex>> const>
      ...
      std::shared_ptr<sopt::OperatorFunction<Vector<t_complex>> const>
        init_grid_degrid_operator_2d(const sopt::mpi::Communicator &comm,
          ...
          return gpu::operators::init_grid_degrid_operator_2d(
              comm, uv_vis.u, uv_vis.v, uv_vis.w, uv_vis.weights, imsizey, imsizex, oversample_ratio,
              power_iters, power_tol, kernel, Ju, Jv, w_term, energy_chirp_fraction,
              energy_kernel_fraction);
        }
#endif
    }
