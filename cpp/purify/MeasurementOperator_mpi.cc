...
namespace purify {
  namespace mpi {
    ...
    MeasurementOperator::MeasurementOperator(const purify::MeasurementOperator & measurements,
        const sopt::mpi::Communicator & comm) : purify::MeasurementOperator(measurements) {
      MeasurementOperator::create_mask(comm);
    }
...
#pragma omp parallel for
    ...
    sopt::LinearTransform<sopt::Vector<sopt::t_complex>>
      linear_transform(MeasurementOperator const &measurements, t_uint nvis,
        ...
        auto direct = [&measurements, width, height, comm](Vector<t_complex> &out, Vector<t_complex> const &x) {
          assert(x.size() == width * height);
          auto const image = Image<t_complex>::Map(x.data(), height, width);
          out = measurements.degrid(image, comm);
        };
        auto adjoint
          = [&measurements, width, height, comm](Vector<t_complex> &out, Vector<t_complex> const &x) {
            ...
            image = measurements.grid(x, comm);
