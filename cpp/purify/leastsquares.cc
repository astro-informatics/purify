...
Vector<t_complex> levenberg_marquardt(
    const std::function<Vector<t_complex>(Vector<t_real>, Vector<t_complex>)> model_function,
    const Vector<t_complex>& y, const Vector<t_complex>& x, const Vector<t_real> weights,
    const Vector<t_complex>& initial_params, total_iterations) {
  // Main least squares solving algortihm
  Vector<t_complex> current_params = initial_params;
  t_real lambda = 0.001;
  auto current_model = [&current_params](const Vector<t_real>& x) {
    return model_function(x, current_params);
  };
  ...
  for (t_int i = 0; i < total_iterations; i++) {
    ...
    auto new_params =
        current_params + iteration(current_params, x, model_function, y, weights, lambda);
    auto new_model = [&current_params](const Vector<t_real>& x) {
      return model_function(x, new_params);
    };
    ...
    if (std::abs(chi2 - chi2_new) < 0.001) break;
  }
...
Vector<t_complex> iteration(
    const Vector<t_complex>& current_params, const Vector<t_real>& x,
    const std::function<Vector<t_complex>(Vector<t_real>, Vector<t_complex>)> model_function,
...
Matrix<t_complex> jacobian_matrix(
    const std::function<Vector<t_complex>(Vector<t_real>)> model_function, const Vector<t_real> x0,
...
Matrix<t_complex> alpha_matrix(const Matrix<t_complex>& J, const Vector<t_real>& weights,
  ...
  for (t_int i = 0; i < alpha.rows(); i++) {
    for (t_int j = 0; j < alpha.cols(); j++) {
      ...
      if (i == j) alpha(i, j) = alpha(i, j) * (1 + lambda);
    }
  }
  return alpha;
}
...
Vector<t_complex> residual_calculation(
    const Vector<t_complex>& y0, const Vector<t_real>& x,
    const std::function<Vector<t_complex>(Vector<t_complex>)> model_function,
    const Vector<t_real>& weights) {
  //
