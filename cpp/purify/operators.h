...
namespace purify {
  ...
  namespace details {
    ...
    Sparse<t_complex> init_gridding_matrix_2d(const Vector<t_real> &u, 
        const Vector<t_real> &v, const t_uint & imsizey_, const t_uint & imsizex_, const t_uint oversample_ratio, const t_uint resample_factor, const std::function<t_real(t_real)> kernelu,
      ...
      t_uint p;
      t_uint index;
      auto omega_to_k = [](const Vector<t_real> &omega) {return omega.unaryExpr(std::ptr_fun<double, double>(std::floor));};
      const Vector<t_real> k_u
        = omega_to_k(u - Vector<t_real>::Constant(rows, Ju * 0.5));
      const Vector<t_real> k_v
        = omega_to_k(v - Vector<t_real>::Constant(rows, Jv * 0.5));
...
#pragma omp simd collapse(3)
    ...
    Image<t_real>
      init_correction2d(const t_real &oversample_ratio, const t_uint & imsizey_, const t_uint imsizex_, const std::function<t_real(t_real)> ftkernelu,
...
*/
    ...
    template <class T>
      t_real power_method(const sopt::LinearTransform<T> & op, const t_uint &niters, const t_real &relative_difference, const T &initial_vector) {
        /*
           Attempt at coding the power method, returns thesqrt of the largest eigen value of a linear operator composed with its adjoint
        ...
        for(t_int i = 0; i < niters; ++i) {
          ...
          if(relative_difference * relative_difference > std::abs(old_value - estimate_eigen_value) / old_value) {
            old_value = estimate_eigen_value;
            break;
          }
          old_value = estimate_eigen_value;
        }
        PURIFY_DEBUG("Converged to norm = {}, relative difference < {}", std::sqrt(old_value), relative_difference);
  ...
  namespace operators {
    ...
    template <class T>
      std::tuple<sopt::OperatorFunction<T>, sopt::OperatorFunction<T>> init_gridding_matrix_2d(const sopt::mpi::Communicator comm, const Vector<t_real> &u, 
          const Vector<t_real> &v, const t_uint & imsizey_, const t_uint & imsizex_, const t_uint oversample_ratio, const t_uint resample_factor, const std::function<t_real(t_real)> kernelu,
          const std::function<t_real(t_real)> kernelv, const t_uint Ju = 4, const t_uint Jv = 4) {

        Sparse<t_complex> interpolation_matrix = details::init_gridding_matrix_2d(u, v, imsizey_, imsizex_, oversample_ratio, resample_factor, kernelu, kernelv, Ju, Jv);
        ...
        return std::make_tuple([=](T & output, const T & input) {
            if (comm.is_root()){
    ...
    template <class T>
      std::tuple<sopt::OperatorFunction<T>, sopt::OperatorFunction<T>> init_broadcaster(const sopt::mpi::Communicator comm) {
    ...
    template <class T>
      std::tuple<sopt::OperatorFunction<T>, sopt::OperatorFunction<T>> init_all_sum_all(const sopt::mpi::Communicator comm) {
        return std::make_tuple([=](T & output, const T & input) {output = input;},
    ...
    template <class T>
      std::tuple<sopt::OperatorFunction<T>, sopt::OperatorFunction<T>> init_gridding_matrix_2d(const Vector<t_real> &u, 
          const Vector<t_real> &v, const t_uint & imsizey_, const t_uint & imsizex_, const t_uint oversample_ratio, const t_uint resample_factor, const std::function<t_real(t_real)> kernelu,
          const std::function<t_real(t_real)> kernelv, const t_uint Ju = 4, const t_uint Jv = 4) {

        const Sparse<t_complex> interpolation_matrix = details::init_gridding_matrix_2d(u, v, imsizey_, imsizex_, oversample_ratio, resample_factor, kernelu, kernelv, Ju, Jv);
        const Sparse<t_complex> adjoint = interpolation_matrix.adjoint();

        return std::make_tuple([=](T & output, const T & input) {output = utilities::sparse_multiply_matrix(interpolation_matrix, input);},
            [=](T & output, const T & input) {output = utilities::sparse_multiply_matrix(adjoint, input);});
      }
...
#pragma omp simd collapse(2)
          for (t_uint j = 0; j < imsizey_; j++) {
            for (t_uint i = 0; i < imsizex_; i++) {
              ...
              const t_uint output_index = utilities::sub2ind(y_start + j, x_start + i, ftsizev_, ftsizeu_);
...
#pragma omp simd collapse(2)
    ...
    template <class T>
      std::tuple<sopt::OperatorFunction<T>, sopt::OperatorFunction<T>> 
      init_weights_(const Vector<t_complex> & weights, const std::string & weighting_type_, const t_real & R_, const t_uint &imsizex_, const t_uint &imsizey_, const t_real &oversample_ratio, const Vector<t_real> &u, const Vector<t_real> &v) {
        ...
        auto direct = [=](T & output, const T & x) {
          output = W.array() * x.array();
        };
        auto indirect = [=]( T & output, const T & x) {
          output = W.conjugate().array() * x.array();
        };
        return std::make_tuple(direct, indirect);
      }
    ...
    template <class T>
      std::tuple<sopt::OperatorFunction<T>, sopt::OperatorFunction<T>> 
      init_FFT_2d(const t_uint & imsizey_, const t_uint & imsizex_, const t_real & oversample_factor_, const t_real & resample_factor, 
          const std::string &fftw_plan_flag_ = "measure") {
        const std::shared_ptr<FFTOperator> fftop = std::make_shared<FFTOperator>(FFTOperator::FFTOperator());
        ...
        auto direct = [=](T & output, const T & input){
          ...
          output_image = utilities::re_sample_ft_grid(fftop->forward(output_image),
              resample_factor);
          output = Image<t_complex>::Map(output_image.data(), output_image.size(), 1);
        };
        auto indirect = [=](T & output, const T & input){
          Matrix<t_complex> output_image = Image<t_complex>::Map(input.data(), std::floor(ftsizev_ * resample_factor), std::floor(ftsizeu_ * resample_factor));
  ...
  namespace measurementoperator {
    template <class T>
      sopt::LinearTransform<T> init_degrid_operator_2d(const Vector<t_real> &u, 
          const Vector<t_real> &v, const t_uint & imsizey, const t_uint & imsizex, const t_real oversample_ratio = 2, const t_uint &power_iters = 100, const t_real & power_tol = 1e-4,
          const t_real resample_factor = 1., const std::string & kernel = "kb", const t_uint Ju = 4, const t_uint Jv = 4, const std::string & ft_plan = "measure") {
        /*
         *  Returns linear transform that is the standard degridding operator
         */

        std::function<t_real(t_real)> kernelu, kernelv, ftkernelu, ftkernelv;
        std::tie(kernelu, kernelv, ftkernelu, ftkernelv) = purify::create_kernels(kernel, Ju, Jv, imsizey, imsizex, oversample_ratio);
        ...
        const Image<t_real> S = purify::details::init_correction2d(oversample_ratio, imsizey, imsizex, ftkernelu, ftkernelv);
        std::tie(directZ, indirectZ) = purify::operators::init_zero_padding_2d<T>(S, oversample_ratio);
        std::tie(directFFT, indirectFFT) = purify::operators::init_FFT_2d<T>(imsizey, imsizex, oversample_ratio, resample_factor, ft_plan);
        std::tie(directG, indirectG) = purify::operators::init_gridding_matrix_2d<T>(u, v, imsizey, imsizex, oversample_ratio, resample_factor, kernelv, kernelu, Ju, Jv);
        ...
        const t_real op_norm = details::power_method<T>(measure_op, power_iters, power_tol, T::Random(imsizex * imsizey));
        std::tie(directZ, indirectZ) = purify::operators::init_zero_padding_2d<T>(S/op_norm, oversample_ratio);
    ...
    template <class T>
      sopt::LinearTransform<T> init_degrid_weighted_operator_2d(const Vector<t_real> &u, 
          const Vector<t_real> &v, const Vector<t_complex> & weights, const t_uint & imsizey, const t_uint & imsizex, const t_real oversample_ratio = 2, const t_uint &power_iters = 100, const t_real & power_tol = 1e-4,
          const t_real resample_factor = 1., const std::string & kernel = "kb", const t_uint Ju = 4, const t_uint Jv = 4, const std::string & ft_plan = "measure", const std::string & weighting_type = "none", const t_real & R = 0) {
        /*
         *  Returns linear transform that is the standard degridding operator
         */
        ...
        std::tie(directFFT, indirectFFT) = purify::operators::init_FFT_2d<T>(imsizey, imsizex, oversample_ratio, resample_factor, ft_plan);
        std::tie(directG, indirectG) = purify::operators::init_gridding_matrix_2d<T>(u, v, imsizey, imsizex, oversample_ratio, resample_factor, kernelv, kernelu, Ju, Jv);
        std::tie(directW, indirectW) = purify::operators::init_weights_<T>(weights, weighting_type, R, imsizex, imsizey, oversample_ratio, u, v);
...
#ifdef PURIFY_MPI
    template <class T>
      sopt::LinearTransform<T> init_degrid_operator_2d(const sopt::mpi::Communicator & comm, const Vector<t_real> &u, 
          const Vector<t_real> &v, const t_uint & imsizey, const t_uint & imsizex, const t_real oversample_ratio = 2, const t_uint &power_iters = 100, const t_real & power_tol = 1e-4,
          const t_real resample_factor = 1., const std::string & kernel = "kb", const t_uint Ju = 4, const t_uint Jv = 4, const std::string & ft_plan = "measure") {
        /*
    ...
    template <class T>
      sopt::LinearTransform<T> init_degrid_weighted_operator_2d(const sopt::mpi::Communicator & comm, const Vector<t_real> &u, 
          const Vector<t_real> &v, const Vector<t_complex> & weights, const t_uint & imsizey, const t_uint & imsizex, const t_real oversample_ratio = 2, const t_uint &power_iters = 100, const t_real & power_tol = 1e-4,
          const t_real resample_factor = 1., const std::string & kernel = "kb", const t_uint Ju = 4, const t_uint Jv = 4, const std::string & ft_plan = "measure", const std::string & weighting_type = "none", const t_real & R = 0) {
        /*
        ...
        std::tie(directZ, indirectZ) = purify::operators::init_zero_padding_2d<T>(S/op_norm, oversample_ratio);
        direct = sopt::chained_operators<T>(directW, directG, directFFT, directZ);
        indirect = sopt::chained_operators<T>(indirectAllsumall, indirectZ, indirectFFT, indirectG, indirectW);
        return {direct, indirect};
      }
    template <class T>
      sopt::LinearTransform<T> init_degrid_operator_2d_mpi(const sopt::mpi::Communicator & comm, const Vector<t_real> &u, 
          const Vector<t_real> &v, const t_uint & imsizey, const t_uint & imsizex, const t_real oversample_ratio = 2, const t_uint &power_iters = 100, const t_real & power_tol = 1e-4,
          const t_real resample_factor = 1., const std::string & kernel = "kb", const t_uint Ju = 4, const t_uint Jv = 4, const std::string & ft_plan = "measure") {
        /*
         *  Returns linear transform that is the standard degridding operator with distributed Fourier grid
         */
        ...
        std::tie(directZ, indirectZ) = purify::operators::init_zero_padding_2d<T>(S, oversample_ratio);
        std::tie(directFFT, indirectFFT) = purify::operators::init_FFT_2d<T>(imsizey, imsizex, oversample_ratio, resample_factor, ft_plan);
        std::tie(directG, indirectG) = purify::operators::init_gridding_matrix_2d<T>(comm, u, v, imsizey, imsizex, oversample_ratio, resample_factor, kernelv, kernelu, Ju, Jv);
        ...
        const sopt::LinearTransform<T> measure_op = {direct, indirect};
        const t_real op_norm = details::power_method<T>(measure_op, power_iters, power_tol, comm.broadcast<T>(T::Random(imsizex * imsizey)));
        std::tie(directZ, indirectZ) = purify::operators::init_zero_padding_2d<T>(S/op_norm, oversample_ratio);
        if (comm.is_root()) {
          direct = sopt::chained_operators<T>(directG, directFFT, directZ);
          indirect = sopt::chained_operators<T>(indirectBroadcast, indirectZ, indirectFFT, indirectG);
    ...
    template <class T>
      sopt::LinearTransform<T> init_degrid_weighted_operator_2d_mpi(const sopt::mpi::Communicator & comm, const Vector<t_real> &u, 
          const Vector<t_real> &v, const Vector<t_complex> & weights, const t_uint & imsizey, const t_uint & imsizex, const t_real oversample_ratio = 2, const t_uint &power_iters = 100, const t_real & power_tol = 1e-4,
          const t_real resample_factor = 1., const std::string & kernel = "kb", const t_uint Ju = 4, const t_uint Jv = 4, const std::string & ft_plan = "measure", const std::string & weighting_type = "none", const t_real & R = 0) {
        /*
         *  Returns linear transform that is the weighted degridding operator with a distributed Fourier grid
         */
        ...
        std::tie(directFFT, indirectFFT) = purify::operators::init_FFT_2d<T>(imsizey, imsizex, oversample_ratio, resample_factor, ft_plan);
        std::tie(directG, indirectG) = purify::operators::init_gridding_matrix_2d<T>(comm, u, v, imsizey, imsizex, oversample_ratio, resample_factor, kernelv, kernelu, Ju, Jv);
        std::tie(directW, indirectW) = purify::operators::init_weights_<Vector<t_complex>>(weights, weighting_type, R, imsizex, imsizey, oversample_ratio, u, v);
        sopt::OperatorFunction<T> direct;
        sopt::OperatorFunction<T> indirect;
        if (comm.is_root()) {
          direct = sopt::chained_operators<T>(directW, directG, directFFT, directZ);
          indirect = sopt::chained_operators<T>(indirectBroadcast, indirectZ, indirectFFT, indirectG, indirectW);
        } else {
          direct = sopt::chained_operators<T>(directW, directG);
          indirect = sopt::chained_operators<T>(indirectBroadcast, indirectG, indirectW);
        }
        const sopt::LinearTransform<T> measure_op = {direct, indirect};
        const t_real op_norm = comm.broadcast<t_real>(details::power_method<T>(measure_op, power_iters, power_tol, comm.broadcast<T>(T::Random(imsizex * imsizey))));
        std::tie(directZ, indirectZ) = purify::operators::init_zero_padding_2d<T>(S/op_norm, oversample_ratio);
        if (comm.is_root()) {
          direct = sopt::chained_operators<T>(directW, directG, directFFT, directZ);
          indirect = sopt::chained_operators<T>(indirectBroadcast, indirectZ, indirectFFT, indirectG, indirectW);
