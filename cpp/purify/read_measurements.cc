...
utilities::vis_params read_measurements(const std::vector<std::string> &names, const bool w_term, const stokes pol) {
  ...
  else
  {
    ...
    if (format == ".ms")
    {
      ...
      if(not boost::filesystem::is_directory(file_path))
        throw std::runtime_error(names.at(i) + " is not a directory, as expected for a measurement set.");
    }
    else if (format == ".vis")
    {
  ...
  if(found_files.size() == 0)
   throw std::runtime_error("No files found, all files are missing!");
  switch(format_type){
    case(format::vis):
      {
      if(pol != stokes::I)
...
read_measurements(const std::vector<std::string> &names, sopt::mpi::Communicator const &comm, 
  ...
  if(comm.is_root()) {
    auto result = read_measurements(names, w_term, pol);
