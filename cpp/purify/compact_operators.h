...
template <class T>
sopt::OperatorFunction<T> base_mpi_grid_degrid_operator_2d(
    const sopt::mpi::Communicator &comm, const Vector<t_real> &u, const Vector<t_real> &v,
  ...
  GTG = purify::operators::init_gridding_degridding_matrix_2d<T>(comm, u, v, weights, imsizey,
                                                                 imsizex, oversample_ratio, kernelu,
                                                                 kernelv, Ju, Jv, cellx, celly);
  if (comm.is_root())
...
sopt::OperatorFunction<T> init_gridding_degridding_matrix_2d(
    ...
    const t_uint Ju = 4, const t_uint Jv = 4, const t_real &cellx = 1, const t_real &celly = 1) {
...
template <class T>
sopt::OperatorFunction<T> base_grid_degrid_operator_2d(
...
sopt::OperatorFunction<T> init_grid_degrid_operator_2d(
  ...
  const t_real op_norm = sopt::algorithm::power_method<T>({direct, N, id, N}, power_iters,
                                                          power_tol, T::Random(imsizex * imsizey));
...
sopt::OperatorFunction<T> init_grid_degrid_operator_2d(
  ...
  std::array<t_int, 3> N = {0, 1, static_cast<t_int>(imsizey * imsizex)};
  const sopt::OperatorFunction<T> phiTphi = purify::operators::base_grid_degrid_operator_2d<T>(
      u, v, w, weights, imsizey, imsizex, oversample_ratio, kernel, Ju, Jv, ft_plan,
      cellx, celly);
  const auto allsumall = purify::operators::init_all_sum_all<T>(comm);
  auto direct = sopt::chained_operators<T>(allsumall, phiTphi);
  auto id = [](T &out, const T &in) { out = in; };
  const t_real op_norm = sopt::algorithm::power_method<T>(
      {direct, N, id, N}, power_iters, power_tol, comm.broadcast<T>(T::Random(imsizex * imsizey)));
...
sopt::OperatorFunction<T> init_grid_degrid_operator_2d(
  ...
  return init_grid_degrid_operator_2d<T>(comm, uv_vis.u, uv_vis.v, uv_vis.w, uv_vis.weights,
                                         ...
                                         kernel, Ju, Jv, w_term, cell_x, cell_y);
}
template <class T>
...
sopt::OperatorFunction<T> init_grid_degrid_operator_2d_mpi(
  ...
  return init_grid_degrid_operator_2d_mpi<T>(comm, uv_vis.u, uv_vis.v, uv_vis.w, uv_vis.weights,
                                             ...
                                             power_tol, kernel, Ju, Jv, w_term, cell_x, cell_y);
}
#endif
template <class T>
sopt::OperatorFunction<T> init_psf_convolve_2d(
    const std::shared_ptr<sopt::LinearTransform<T> const> &degrid_grid, const t_uint &imsizey,
    const t_uint &imsizex, const t_uint &power_iters = 100, const t_real &power_tol = 1e-4) {
  std::array<t_int, 3> N = {0, 1, static_cast<t_int>(imsizey * imsizex)};
...
#pragma omp parallel for
    for (t_uint i = 0; i < input.size(); i++) out(i) = ft_psf(i) * input(i);
  };
  const auto psf_convolve = sopt::chained_operators<T>(ifftop, ft_psf_multiply, fftop);
  const auto id = [](T &out, const T &in) { out = in; };
  const t_real op_norm = sopt::algorithm::power_method<T>({psf_convolve, N, id, N}, power_iters,
                                                          power_tol, T::Random(imsizex * imsizey));
  const auto operator_norm = purify::operators::init_normalise<T>(op_norm * op_norm);
