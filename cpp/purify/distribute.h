...
std::tuple<std::vector<t_int>, std::vector<t_real>> kmeans_algo(const Vector<t_real> &w,
                                                                ...
                                                                const t_int iters, const std::function<t_real(t_real)> &cost = [](t_real x) { return x * x; });
#ifdef PURIFY_MPI
...
std::tuple<std::vector<t_int>, std::vector<t_real>> kmeans_algo(
    ...
    sopt::mpi::Communicator const &comm, const std::function<t_real(t_real)> &cost = [](t_real x) { return x * x; });
#endif
