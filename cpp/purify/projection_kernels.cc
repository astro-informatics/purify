...
std::function<t_complex(t_real, t_real, t_real)> const w_projection_kernel_approx(
    const t_real &cellx, const t_real &celly, const t_uint &imsizex, const t_uint &imsizey,
...
std::function<t_complex(t_real, t_real, t_real)> const w_projection_kernel_sphere(
    const t_real &cellx, const t_real &celly, const t_uint &imsizex, const t_uint &imsizey,
    const t_real &oversample_ratio) {
  const t_real du = pixel_to_lambda(cellx, imsizex, oversample_ratio);
  const t_real dv = pixel_to_lambda(celly, imsizey, oversample_ratio);
...
Matrix<t_complex> projection(const std::function<t_real(t_real)> kernelu,
                             const std::function<t_real(t_real)> kernelv,
                             const std::function<t_complex(t_real, t_real, t_real)> kernelw,
                             const t_real u, const t_real v, const t_real w, const t_int &Ju,
                             const t_int &Jv, const t_int &Jw) {
