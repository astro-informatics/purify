...
int main(int, char **) {
  ...
  auto measurements_transform = sopt::linear_transform<Vector<t_complex>>(
    direct, {0, 1, static_cast<t_int>(width * height)},
    adjoint, {0, 1, static_cast<t_int>(width * height)}
  );

  sopt::wavelets::SARA const sara{std::make_tuple("Dirac", 4u), std::make_tuple("DB1", 4u), std::make_tuple("DB2", 4u), 
  ...
  //t_real epsilon = utilities::calculate_l2_radius(dirty_measurements, noise_rms); //Calculation of l_2 bound following SARA paper
  t_real epsilon = std::sqrt(uv_data.vis.size()) * noise_rms;
  auto purify_gamma = (Psi.adjoint() * (measurements_transform.adjoint() * dirty_measurements)).real().cwiseAbs().maxCoeff() * beta;
  ...
  std::tuple<Vector<t_complex>, Vector<t_complex>> const estimates(Vector<t_complex>::Zero(dirty_measurements.size()), Vector<t_complex>::Zero(dirty_measurements.size()));  
