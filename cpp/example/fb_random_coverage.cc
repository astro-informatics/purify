...
void forward_backward(const std::string &name, const Image<t_complex> &M31,
  ...
  while (!canvas->is_closed() or !new_canvas->is_closed()) canvas->wait();
#endif
}
...
int main(int, char **) {
  ...
  auto const sky_measurements = gpu::measurementoperator::init_degrid_operator_2d(
      uv_data, M31.rows(), M31.cols(), cellsize, cellsize, 2, 500, 0.0001,
  ...
  forward_backward(name + "30", M31, kernel, 4, uv_data, sigma, std::make_tuple(w_term, cellsize));
  return 0;
}
