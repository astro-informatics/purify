...
int main(int, char **) {
  ...
  MeasurementOperator measurements(uv_data, 4, 4, "kb_min", width, height, 20, over_sample, cellsize, cellsize, "none");
  ...
  std::string const psf_model_fits = output_filename(name + "_psf_model_"+ weighting + "_clean.fits");
  ...
  Image<t_complex> residual = measurements.grid(((uv_data.vis - measurements.degrid(model)).array() * uv_data.weights.array().real()).matrix() ).array();
  header.pix_units = "JY/BEAM";
