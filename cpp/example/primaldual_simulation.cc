...
\
  ...
  sopt::Vector<t_complex> rand = sopt::Vector<t_complex>::Random(uv_data.vis.size()*nlevels);
  ...
  rand = sopt::Vector<t_complex>::Random(uv_data.vis.size());
  ...
  PURIFY_MEDIUM_LOG("Kappa {}", kappa);
  PURIFY_MEDIUM_LOG("Tau {}", tau);
  PURIFY_MEDIUM_LOG("Sigma1 {}", sigma1);
  PURIFY_MEDIUM_LOG("Sigma2 {}", sigma2);
