...
namespace purify {
  ...
  class MeasurementOperator {
    public:
      ...
      Vector<t_complex> degrid(Image <t_complex> eigen_image, const MeasurementOperator::operator_params st);
      //! Gridding operator that grids image from visibilities
      Image<t_complex> grid(Vector <t_complex> visibilities, const MeasurementOperator::operator_params st);
      ...
      void writefits2d(Image<t_real> image, const std::string fits_name, const bool overwrite);
      ...
      MeasurementOperator::operator_params init_nufft2d(Vector<t_real> u, Vector<t_real> v, const t_int Ju, const t_int Jv, const std::string kernel_name, const t_int imsizex, const t_int imsizey, const t_int oversample_factor);
      //! Function that chooses what gridding kernel to calculate
      t_real choose_kernel(const t_real x, const t_int J, const std::string kernel_name);
      ...
      t_real ftkaiser_bessel(const t_real x, const t_int J);
      //! Gaussian kernel
