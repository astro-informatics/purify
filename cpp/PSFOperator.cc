...
namespace purify {
  ...
  PSFOperator::PSFOperator(const Image<t_complex>& point_spread_function)
      : ftsize_u(static_cast<t_int>(point_spread_function.cols())), ftsize_v(static_cast<t_int>(point_spread_function.rows()))
  {
  ...
  Image<t_complex> PSFOperator::convolution_with_psf(const Image<t_complex>& image, const Image<t_complex>& ft_of_psf){
  	/*
    ...
    return fftop.inverse(fftop.forward(padded_image).array() * ft_of_psf).array().block(y_start, x_start, imsizey, imsizex);
