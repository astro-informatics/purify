...
BENCHMARK_REGISTER_F(AlgoFixtureMPI, PadmmDistributeImage)
    ...
    ->Args({1024, static_cast<t_int>(1e8), 4, 10, 1})
    ->Args({1024, static_cast<t_int>(1e9), 4, 10, 1})
    ->Args({2048, static_cast<t_int>(1e6), 4, 10, 2}) //1?
    ->Args({2048, static_cast<t_int>(1e7), 4, 10, 2}) //1?
    ->Args({4096, static_cast<t_int>(1e6), 4, 10, 2}) //1?
    ->Args({4096, static_cast<t_int>(1e7), 4, 10, 2}) //1?
    ->UseManualTime()
    ->MinTime(120.0)
    ->MinWarmUpTime(10.0)
    ->Repetitions(3)  //->ReportAggregatesOnly(true)
