...
BENCHMARK_DEFINE_F(PadmmFixtureMPI, ApplyAlgo1)(benchmark::State &state) {
  ...
  //     factory::l2_convergence_factory<t_complex>(factory::ConvergenceType::mpi_local,
  //     padmm_weak));
  // padmm->objective_convergence(
  //     factory::l1_convergence_factory<t_complex>(factory::ConvergenceType::mpi_local,
  //     padmm_weak));

  auto const wavelets = factory::wavelet_operator_factory<Vector<t_complex>>(
      factory::distributed_wavelet_operator::mpi_sara, m_sara, m_imsizey, m_imsizex);
  t_real const sigma =
      m_world.broadcast(0.016820222945913496) * std::sqrt(2);  // see test_parameters file
  auto const padmm = factory::padmm_factory<sopt::algorithm::ImagingProximalADMM<t_complex>>(
      factory::algo_distribution::mpi_distributed, m_measurements1, wavelets, m_uv_data, sigma,
...
BENCHMARK_DEFINE_F(PadmmFixtureMPI, ApplyAlgo3)(benchmark::State &state) {
  ...
  t_real const sigma =
      m_world.broadcast(0.016820222945913496) * std::sqrt(2);  // see test_parameters file
  auto const padmm = factory::padmm_factory<sopt::algorithm::ImagingProximalADMM<t_complex>>(
      factory::algo_distribution::mpi_distributed, m_measurements3, wavelets, m_uv_data, sigma,
      m_imsizey, m_imsizex, m_sara.size(), state.range(3) + 1, true, true, false, 1e-3, 1e-2, 50,
      1.0, 1.0);
