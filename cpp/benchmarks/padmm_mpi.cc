...
public:
  void SetUp(const ::benchmark::State& state) {
    ...
    bool newMeasurements = b_utilities::updateMeasurements(state.range(1), m_uv_data, m_epsilon, newImage, m_image, m_world);
...
BENCHMARK_DEFINE_F(PadmmFixtureMPI, Apply)(benchmark::State &state) {
  ...
  t_real gamma = (Psi.adjoint() * (m_measurements->adjoint() * m_uv_data.vis)).cwiseAbs().maxCoeff() * 1e-3;
  ...
  sopt::ScalarRelativeVariation<t_complex> conv(
	padmm->relative_variation(), padmm->relative_variation(), "Objective function");
  std::weak_ptr<decltype(padmm)::element_type> const padmm_weak(padmm);
  padmm->residual_convergence([padmm_weak, this]
  ...
  padmm->objective_convergence([padmm_weak, conv, this]
			       (Vector<t_complex> const &x, Vector<t_complex> const &) mutable -> bool {
				 auto const padmm = padmm_weak.lock();
				 auto const result
				   = this->m_world.all_reduce<uint8_t>(
