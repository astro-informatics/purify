clear
Ju = 4;
Jv = Ju;
cellsize = 0.3; %arcsec
imsize = [128, 128];
oversample_rate = 2; %most of the kernel parameters are optimized for K/N=2
FTsize = imsize*oversample_rate;
kernel_type = 'kb';
stringname = 'at166B.3C129.c0.vis';
parallel_type = 2; %I think choosing 2 is faster than choosing 1

Y = importdata(stringname);
U = Y(:,1);
V = Y(:,2);
vis = Y(:,3)+1j*Y(:,4);
%m = sqrt(max(max(U.^2+V.^2)))/3;
m = 180/pi*3600/cellsize;
U = [U;-U]/m * FTsize(1);
V = [V;-V]/m * FTsize(2);
vis = [vis; conj(vis)];

st = purify_mtlb_init_nufft([U, V], imsize, oversample_rate, kernel_type, [Ju, Jv], parallel_type);
fftdata = st.G' * vis;
fftdata = transpose(reshape(fftdata, FTsize(1), FTsize(2)));

im = ifftshift(ifft2(fftdata));%*sqrt(fftsize(1)*fftsize(2));
im_deconv = im./st.S;

figure
realim_deconv = real(im_deconv);
image(realim_deconv/max(max(realim_deconv))*100);

figure
realim = real(im);
image(realim/max(max(realim))*100);
figure
imagim = imag(im);
image(imagim);
realname = sprintf('at166BtestReal_deconvJ%d%s.fits',Ju, kernel_type);
fitswrite(flipud(realim_deconv), realname)
realname = sprintf('at166BtestRealJ%d%s.fits',Ju, kernel_type);
fitswrite(flipud(realim), realname)
%imagname = sprintf('at166BtestImagJ%d%s.fits',Ju, kernel_type);
%fitswrite(imagim, imagname)