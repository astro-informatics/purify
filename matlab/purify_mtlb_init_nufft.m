...
switch kernelname
    case 'kb'
        ...
        st.gridding_correction = 1./((ft_kb_kernel(X./FTsize(1)-0.5, alphau, Ju).*ft_kb_kernel(Y/FTsize(2)-0.5, alphav, Jv)));        
        %storing kernels
    ...
    case 'gauss'
        ...
        st.gridding_correction = 1./(ftgauss_kernel(X./FTsize(1)-0.5,sigmau).*ftgauss_kernel(Y/FTsize(2) - 0.5,sigmav));
        %storing kernel
    ...
    case 'box'
        %Simple binning with no correction.
        Ju = 1;
        Jv = 1;
        kernelu = @ (u, j) 1;
        kernelv = @ (v, j) 1;
